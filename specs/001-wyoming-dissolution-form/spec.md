...
- **FR-003**: The system MUST fetch the submitted homepage and discover up to four additional candidate pages (legal/about/contact/terms) on the same registered domain, respecting robots.txt disallow directives for each path. Per-path compliance means: if a discovered sub-page's path is disallowed by robots.txt, that sub-page is skipped individually and the remaining candidate pages are still fetched; only a disallowed homepage path blocks the entire analysis run (and triggers the honest error + manual-entry fallback). A disallowed sub-page MUST NOT prevent fetching of other allowed sub-pages.
...
- **FR-008**: Every AI-extracted field MUST be validated against a Zod schema before entering application state; fields failing validation are treated as absent. Evidence snippet verification MUST use the following normalization: collapse all whitespace sequences (`\s+`) to a single space and trim leading/trailing whitespace from both the `evidence` string and the `rawText` before the substring check. This normalization rule is the canonical definition for all implementation and testing purposes.
...
- **FR-020**: Every screen that renders form data MUST also render a `LegalDisclaimer` component identifying the product as document preparation only, not legal advice. The screens that render form data are: `/analyze` (progress screen, because the submitted URL is visible and extraction is in progress), `/review` (all sections: extraction board, gap-completion, certification, preview), and `/done` (which personalises content with the company name). The landing page (`/`) is exempt because it does not yet render any extracted or confirmed form data. The disclaimer text MUST NOT contain phrases that claim the product advises on, interprets, or guarantees legal outcomes — specifically, strings matching patterns such as "legal advice", "we advise", "legally guarantees", "ensures compliance", or "this constitutes legal counsel" are prohibited. The prohibition is a testable requirement: automated string search across all UI component text and AI prompt strings MUST find zero occurrences of these patterns.
- **FR-021**: No scraped content, extracted data, or generated PDFs may be persisted server-side beyond the lifetime of the originating request. Server-side logs MUST record only: request latency (ms), HTTP status code, and error class name (e.g., `robots_disallowed`, `ai_timeout`). The following are explicitly prohibited from appearing in any log line, structured log field, or error message: the URL of any scraped page, any page body text or fragment, any extracted field value, the company name, any PDF bytes or text content, and any client IP address in plaintext. A code review or automated log-scanning tool MUST be able to verify this prohibition by confirming that only the three permitted fields are written to the logging sink. Exception: the rate-limit counter store (see FR-026) persists `hash(IP) + counter + TTL` only — this is explicitly not a violation of the no-persistence principle because no URL, page content, field value, or form data is stored. This distinction MUST be documented in the implementation plan to prevent the two requirements from being flagged as contradictory.
